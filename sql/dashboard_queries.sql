...
FROM epa_air_quality.gold.permit_status_current
...
FROM epa_air_quality.gold.lead_rolling_weekly_avg
WHERE station_id  = :station_id
...
SELECT
    ...
    0.45                                        AS site_target,
    0.50                                        AS epa_limit
FROM epa_air_quality.gold.pollutant_monthly_stats
WHERE station_id  = :station_id
...
FROM epa_air_quality.gold.pollutant_daily_summary
WHERE station_id     = :station_id
  AND measurement_date BETWEEN :date_from AND :date_to
ORDER BY measurement_date;
...
SELECT
    ...
    actual_breach,
    breach_predicted_3d
FROM epa_air_quality.gold.breach_predictions
...
SELECT
    ...
    CASE
        ...
        ELSE '🟡 APPROACHING'
    END                                                 AS severity
FROM epa_air_quality.gold.exceedance_log
...
FROM epa_air_quality.gold.pollutant_daily_summary d
LEFT JOIN epa_air_quality.gold.breach_predictions p
...
FROM epa_air_quality.salesforce.epa_limits
WHERE station_id = :station_id
ORDER BY pollutant;
