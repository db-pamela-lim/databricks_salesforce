...
with tab_schools:
    ...
    st.markdown('<div class="section-header">School Community Air Quality Advisories</div>',
                unsafe_allow_html=True)
    st.caption(
        "AI-generated advisories for Port Pirie school principals based on Lead in Air "
        "forecast risk. Generated by Databricks `ai_query()` and stored in "
        "`epa_air_quality.gold.school_air_quality_advisories`."
    )

    if df_advisories.empty:
        st.success("✅ No HIGH or MEDIUM risk days in recent history — no advisories have been issued.")
    else:
        df_advisories["advisory_date"] = pd.to_datetime(df_advisories["advisory_date"])
        df_advisories["breach_probability"] = df_advisories["breach_probability"] * 100

        # ── Today's / most recent advisory ───────────────────────────────────
        latest = df_advisories.iloc[0]
        risk    = latest["risk_band"]
        colour  = COLOURS.get(risk, "#95a5a6")
        icon    = "🔴" if risk == "HIGH" else "🟠"

        st.markdown(f"#### {icon} Most Recent Advisory — {latest['advisory_date'].strftime('%A %d %B %Y')}")
        ...
        with col_badge:
            ...
            <div class="metric-card" style="border-top: 4px solid {colour};">
                ...
                <div class="metric-value" style="color:{colour};">{risk}</div>
                <div class="metric-sub">
                    7-day avg: <b>{latest['rolling_7day_avg']:.3f} μg/m³</b><br>
        ...
        with col_stats:
            st.markdown(f"""
            <div style="background:#fff8f0; border-left:4px solid {colour};
                        padding:16px; border-radius:4px; font-size:15px; line-height:1.6;">
                {latest['advisory_text']}
            </div>
            """, unsafe_allow_html=True)
            st.caption(f"Generated at {pd.to_datetime(latest['generated_at']).strftime('%d %b %Y %H:%M')} · Databricks AI Functions · Model: meta-llama-3-3-70b-instruct")

        st.markdown("---")

        # ── Risk trend for context ────────────────────────────────────────────
        st.markdown('<div class="section-header">Advisory History</div>', unsafe_allow_html=True)

        fig_adv = go.Figure()
        for band, marker in [("HIGH", "circle"), ("MEDIUM", "diamond")]:
            ...
            if not sub.empty:
                fig_adv.add_trace(go.Scatter(
                    ...
                    mode="markers",
                    name=band,
                    marker=dict(color=COLOURS[band], size=12, symbol=marker),
                    customdata=sub[["breach_probability", "advisory_text"]],
                    hovertemplate=(
                        "<b>%{x|%d %b %Y}</b><br>"
        ...
        fig_adv.add_hline(y=0.45, line_dash="dot", line_color=COLOURS["target"],
                          annotation_text="Site target 0.45 μg/m³")
        fig_adv.update_layout(
            height=280, margin=dict(t=20, b=20),
            yaxis_title="Lead in Air 7-day avg (μg/m³)",
            xaxis_title=None, hovermode="closest",
            legend=dict(orientation="h", yanchor="bottom", y=1.02),
        )
        st.plotly_chart(fig_adv, use_container_width=True)

        # ── Full advisory log table ───────────────────────────────────────────
        st.markdown('<div class="section-header">All Advisories</div>', unsafe_allow_html=True)

        # Selectable row to read the full advisory text
        selected_date = st.selectbox(
            "Select a date to read the full advisory:",
        ...
        <div style="background:#f8f9fa; border-left:4px solid {sel_colour};
                    padding:16px; border-radius:4px; font-size:14px; line-height:1.7;">
        ...
        # Summary table
        st.dataframe(
            df_advisories[["advisory_date", "risk_band", "rolling_7day_avg", "breach_probability"]]
            .rename(columns={
                "advisory_date":      "Date",
                "risk_band":          "Risk",
                "rolling_7day_avg":   "7-day avg (μg/m³)",
                "breach_probability": "Breach prob (%)",
            })
            .assign(**{"Date": df_advisories["advisory_date"].dt.strftime("%d %b %Y"),
                       "7-day avg (μg/m³)": df_advisories["rolling_7day_avg"].round(3),
                       "Breach prob (%)": df_advisories["breach_probability"].round(0).astype(int)}),
            use_container_width=True, hide_index=True,
        )
