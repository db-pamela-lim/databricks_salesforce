...
# MAGIC %md ## 6. School Air Quality Advisories (empty shell — populated by notebook 04)
# MAGIC
# MAGIC Created here so the table exists before the app or notebook 04 references it.
# MAGIC Notebook 04 MERGEs AI-generated advisory text into this table for each HIGH/MEDIUM risk day.

# COMMAND ----------

spark.sql(f"""
    CREATE TABLE IF NOT EXISTS {CATALOG}.gold.school_air_quality_advisories (
        advisory_date      DATE        NOT NULL COMMENT 'Date the advisory applies to',
        station_id         STRING      NOT NULL COMMENT 'Monitoring station (PTP01)',
        risk_band          STRING      NOT NULL COMMENT 'HIGH or MEDIUM',
        rolling_7day_avg   DOUBLE      COMMENT '7-day rolling Lead in Air average (ug/m3) on this date',
        breach_probability DOUBLE      COMMENT 'ML model probability of breaching 0.45 ug/m3 within 3 days',
        advisory_text      STRING      COMMENT 'AI-generated plain-English advisory for school principals',
        audience           STRING      COMMENT 'Intended audience description',
        generated_at       TIMESTAMP   COMMENT 'When the advisory was generated'
    )
    USING delta
    COMMENT 'AI-generated school community air quality advisories for HIGH and MEDIUM risk days. Generated by Databricks ai_query() in notebook 04. One row per station per date — re-runs update via MERGE.'
""")

print("✓ gold.school_air_quality_advisories table ready")
...
# MAGIC | `gold.school_air_quality_advisories` | — | App — Schools Advisory tab |
# MAGIC
# MAGIC Next step: **04_ml_exceedance_prediction** → predict lead breaches and generate school advisories.